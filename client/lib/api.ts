...
export function mapCard(c: Record<string, unknown>): Card {
  ...
  return {
    ...
    customerCompanyName:   (c.customerCompanyName as string) ?? '',
    ...
    workOrderDetails: (() => {
      const wo = c.workOrderDetails as Record<string, unknown> | null | undefined;
      if (!wo) return undefined;
      return {
        woDate:                     (wo.woDate as string) ?? '',
        ...
        companyPhone:               (wo.companyPhone as string) ?? '',
        companyEmail:               (wo.companyEmail as string) ?? '',
        deliveryDate:               (wo.deliveryDate as string) ?? '',
        deliveryLocation:           (wo.deliveryLocation as string) ?? '',
        deliveryContactName:        (wo.deliveryContactName as string) ?? '',
        deliveryContactNumber:      (wo.deliveryContactNumber as string) ?? '',
        installationCompletionDate: (wo.installationCompletionDate as string) ?? '',
        typeInsulated:              (wo.typeInsulated as boolean) ?? false,
        typeNonInsulated:           (wo.typeNonInsulated as boolean) ?? false,
        skidHollow:                 (wo.skidHollow as boolean) ?? false,
        skidIBeam:                  (wo.skidIBeam as boolean) ?? false,
        indicatorTube:              (wo.indicatorTube as boolean) ?? false,
        indicatorScale:             (wo.indicatorScale as boolean) ?? false,
        ladderInternal:             (wo.ladderInternal as boolean) ?? false,
        ladderExternal:             (wo.ladderExternal as boolean) ?? false,
        supportInternal:            (wo.supportInternal as boolean) ?? false,
        supportExternal:            (wo.supportExternal as boolean) ?? false,
        supply:                     (wo.supply as boolean) ?? false,
        installation:               (wo.installation as boolean) ?? false,
        testingCommissioning:       (wo.testingCommissioning as boolean) ?? false,
        maintenance:                (wo.maintenance as boolean) ?? false,
        jobDescription:             (wo.jobDescription as string) ?? '',
        items:                      (wo.items as import('@/types').WorkOrderItem[]) ?? [],
      };
    })(),
...
function toCardIn(card: Card, performedBy?: number) {
  ...
  return {
    ...
    customer_name:          card.customerName ?? null,
    customer_company_name:  card.customerCompanyName ?? null,
    date:                   card.date,
    ...
    tank_details:           (card.tankDetails ?? []).map(tank => ({
      id: tank.id,
      label: tank.label,
      length: tank.length,
      width: tank.width,
    ...
    work_order_details: card.workOrderDetails ? {
      wo_date:                     card.workOrderDetails.woDate || null,
      customer_id:                 card.workOrderDetails.customerId || null,
      invoice_no:                  card.workOrderDetails.invoiceNo || null,
      invoice_date:                card.workOrderDetails.invoiceDate || null,
      brand:                      card.workOrderDetails.brand || null,
      company_name:                card.workOrderDetails.companyName || null,
      company_contact_name:        card.workOrderDetails.companyContactName || null,
      company_address:             card.workOrderDetails.companyAddress || null,
      company_phone:               card.workOrderDetails.companyPhone || null,
      company_email:               card.workOrderDetails.companyEmail || null,
      delivery_date:               card.workOrderDetails.deliveryDate || null,
      delivery_location:           card.workOrderDetails.deliveryLocation || null,
      delivery_contact_name:       card.workOrderDetails.deliveryContactName || null,
      delivery_contact_number:     card.workOrderDetails.deliveryContactNumber || null,
      installation_completion_date: card.workOrderDetails.installationCompletionDate || null,
      type_insulated:              card.workOrderDetails.typeInsulated,
      type_non_insulated:          card.workOrderDetails.typeNonInsulated,
      skid_hollow:                 card.workOrderDetails.skidHollow,
      skid_i_beam:                 card.workOrderDetails.skidIBeam,
      indicator_tube:              card.workOrderDetails.indicatorTube,
      indicator_scale:             card.workOrderDetails.indicatorScale,
      ladder_internal:             card.workOrderDetails.ladderInternal,
      ladder_external:             card.workOrderDetails.ladderExternal,
      support_internal:            card.workOrderDetails.supportInternal,
      support_external:            card.workOrderDetails.supportExternal,
      supply:                      card.workOrderDetails.supply,
      installation:                card.workOrderDetails.installation,
      testing_commissioning:       card.workOrderDetails.testingCommissioning,
      maintenance:                 card.workOrderDetails.maintenance,
      job_description:             card.workOrderDetails.jobDescription || null,
      items:                       card.workOrderDetails.items ?? [],
    } : null,
