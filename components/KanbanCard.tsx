...
export default function KanbanCard({ card, index, onClick, onDelete, onApprove, onTerminate, onUnterminate, onComplete, onUpdateCard, onAssignUser, onUpdateWorkStatus, userRole, userDepartment, currentList }: Props) {
      ...
      {currentList === 'Installation' && (
        <div className="mt-2">
          {card.completionDocName && (card.completionDocData || card.completionDocUrl) ? (
            <a
              href={card.completionDocData || card.completionDocUrl}
              target="_blank"
              rel="noopener noreferrer"
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  reader.onload = (evt) => {
                    ...
                    const url = `/completion-docs/${card.id}${ext}`;
                    onUpdateCard({ ...card, completionDocName: file.name, completionDocData: data, completionDocUrl: url, updatedAt: new Date().toISOString() });
                  };
        ...
        <div className="mt-3 pt-3 border-t border-gray-200 flex gap-2 flex-wrap">
          {currentList === 'LPO' && !card.approved && onApprove && (
            <button
              ...
              disabled={!card.purchaseOrderDocData}
              className={`flex items-center gap-1 px-3 py-1.5 rounded text-xs font-medium transition-colors ${
                card.purchaseOrderDocData
