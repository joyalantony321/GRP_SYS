...
export default function CardModal({ card, onClose, onUpdate, onDelete, userRole, userName, userDepartment, channel }: Props) {
  ...
  /** Upload handler for Purchase Order doc — callable by any user. */
  const handlePODocUpload = (file: File) => {
    const reader = new FileReader();
    reader.onload = (evt) => {
      const result = evt.target?.result as string;
      const ext = file.name.includes('.') ? file.name.substring(file.name.lastIndexOf('.')) : '';
      const safeBase = (card.quoteNumber || card.id || 'po').replace(/[^a-zA-Z0-9-_]+/g, '-');
      const updated: Card = { ...card, purchaseOrderDocName: file.name, purchaseOrderDocData: result, purchaseOrderDocUrl: `/po-docs/${safeBase}${ext}`, updatedAt: new Date().toISOString() };
      onUpdate(updated);
  ...
  /** Upload handler for Quotation doc — callable by any user. */
  const handleQtnDocUpload = (file: File) => {
    const reader = new FileReader();
    reader.onload = (evt) => {
      const result = evt.target?.result as string;
      const ext = file.name.includes('.') ? file.name.substring(file.name.lastIndexOf('.')) : '';
      const safeBase = (card.quoteNumber || card.id || 'qtn').replace(/[^a-zA-Z0-9-_]+/g, '-');
      const updated: Card = { ...card, quotationDocName: file.name, quotationDocData: result, quotationDocUrl: `/qtn-docs/${safeBase}${ext}`, updatedAt: new Date().toISOString() };
      onUpdate(updated);
      setEditedCard(updated);
    };
    reader.readAsDataURL(file);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      {(card.purchaseOrderDocName && (card.purchaseOrderDocData || card.purchaseOrderDocUrl)) ? (
                        <a
                          href={card.purchaseOrderDocData || card.purchaseOrderDocUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-3 w-full px-4 py-3 bg-amber-50 border-2 border-amber-300 hover:bg-amber-100 hover:border-amber-400 rounded-xl transition-all group"
                        >
                          ...
                          <svg className="w-3.5 h-3.5 text-amber-500 group-hover:text-amber-700 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                        </a>
                      ) : (
                    ...
                    <div>
                      ...
                      {(card.quotationDocName && (card.quotationDocData || card.quotationDocUrl)) ? (
                        <a
                          href={card.quotationDocData || card.quotationDocUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-3 w-full px-4 py-3 bg-purple-50 border-2 border-purple-300 hover:bg-purple-100 hover:border-purple-400 rounded-xl transition-all group"
                        >
                          ...
                          <svg className="w-3.5 h-3.5 text-purple-500 group-hover:text-purple-700 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                        </a>
                      ) : (
                    ...
                    {channel === 'Work Order' && editedCard.list === 'Installation' && (
                      <div className="col-span-1 md:col-span-2">
                        ...
                        {(editedCard.completionDocName && (editedCard.completionDocData || editedCard.completionDocUrl)) ? (
                          <a
                            href={editedCard.completionDocData || editedCard.completionDocUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-3 w-full px-4 py-3 bg-emerald-50 border-2 border-emerald-300 hover:bg-emerald-100 hover:border-emerald-400 rounded-xl transition-all group"
                          >
                            ...
                            <svg className="w-3.5 h-3.5 text-emerald-500 group-hover:text-emerald-700 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                          </a>
                        ) : (
                          <p className="text-sm text-gray-400 italic px-1">No Completion document attached.</p>
                        )}
                        {isEditing && !card.completedAt && (
                          <input
                            type="file"
                            ...
                            onChange={(e) => {
                              ...
                              reader.onload = (evt) => {
                                const result = evt.target?.result as string;
                                const ext = file.name.includes('.') ? file.name.substring(file.name.lastIndexOf('.')) : '';
                                const safeBase = (editedCard.quoteNumber || card.id || 'completion').replace(/[^a-zA-Z0-9-_]+/g, '-');
                                setEditedCard({ ...editedCard, completionDocName: file.name, completionDocData: result, completionDocUrl: `/completion-docs/${safeBase}${ext}` });
                              };
                              reader.readAsDataURL(file);
                            }}
                          />
